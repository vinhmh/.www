...
function load(options) {
    ...
    const file = parse_1.parseFile(`${options.directory}/${options.envFilename}`, options) || {};
    // Parse default values for our env variables
    const defaults = parse_1.parseFile(`${options.directory}/${options.envDefaultsFilename}`, options) || {};
    // Parse variable types for our env variables
    const types = parse_1.parseFile(`${options.directory}/${options.envTypesFilename}`, { ...options, inlineTypes: false }) || {};
    const env = {};
    ...
    for (const [key, intendedType] of Object.entries(types)) {
        ...
        env[key] = type_1.default(env[key], intendedType, options);
    }
