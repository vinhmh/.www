...
class Timer extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.currentUser.startStopTime &&
      this.props.currentUser.isStartTime &&
      !prevProps.currentUser.isStartTime &&
      !this.props.currentUser.isResetTime
    ) {
      this.setState({
        distanceTimeStop: this.state.distanceTimeStop + new Date().getTime() - Date.parse(prevProps.currentUser.startStopTime),
      })
    }
  ...
  interpreterTimeConverter = (prevProps) => {
    ...
    if (this.props.currentUser.isResetTime) {
      ...
      let lastInterpreterSessionTime = null
      let startMeeting = false
      this.handleUpdateAttributesTimeModerator(isStartTime, isResetTime, startStopTime, lastInterpreterSessionTime, startMeeting)
      this.setState({
        distanceTimeStop: 0.0,
        timer: "00:00:00"
      })
    ...
    const difference = getDifferentTime(startDate, endDate, this.state.distanceTimeStop)
  ...
  handleUpdateAttributesTimeModerator(isStartTime, isResetTime, startStopTime, lastInterpreterSessionTime, startMeeting) {
    const { currentUser, members } = this.props;
    members.map(m => {
      if (m.user.meetingID == currentUser.meetingID) {
        const msg = { id: m.id, isStartTime, startStopTime, isResetTime, lastInterpreterSessionTime, startMeeting }
  ...
  handleEventTimeModerator(e) {
    ...
    let lastInterpreterSessionTime = this.props.currentUser.lastInterpreterSessionTime
    let startMeeting = this.props.currentUser.startMeeting
    if (e == "start") {
      isStartTime = true
      isResetTime = false
      startMeeting = true
      if (lastInterpreterSessionTime == null) lastInterpreterSessionTime = new Date()
    }
    else if (e == "stop") {
      isStartTime = false
      ...
      isStartTime = false
      isResetTime = true
      lastInterpreterSessionTime = null
      this.setState({
        distanceTimeStop: 0.0,
        timer: "00:00:00"
      })
        ...
        </span>}
        {
          this.props.currentUser.isTechAssistant && this.state.isShowOptionTimer?
            <div className={tcss.btnGroup}>
              <button onClick={() => this.handleEventTimeModerator("start")}>start</button>
              <button onClick={() => this.handleEventTimeModerator("stop")}>stop</button>
